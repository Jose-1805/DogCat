...
use DogCat\Http\Requests\RequestEntidad;
use DogCat\Models\Correo;
use DogCat\Models\Entidad;
...
class EntidadController extends Controller
{
    ...
    public $identificador_modulo = 11;
    /**
    ...
    public function lista(){
        if (Auth::user()->tieneFuncion($this->identificador_modulo, 'ver', $this->privilegio_superadministrador)){
            $entidad = Veterinaria::select(
                ...
                ->where('veterinarias.entidad','si')
                ->get();

            $table = Datatables::of($entidad);
            ...
            $table = $table->editColumn('direccion',function ($row){
                ...
                if (Auth::user()->tieneFuncion($this->identificador_modulo, 'editar', $this->privilegio_superadministrador)) {
                    if($row->estado == 'aprobada'){
                        $opciones .= "<a title='Editar' href='" . url('entidad/editar/' . $row->id) . "' class='btn btn-xs margin-2 btn-primary'><i class='fa fa-edit'></i></a>";
    ...
    public function crear()
    {
        ...
        $roles = Rol::where('entidades','si')->pluck('nombre','id');
    ...
    public function guardar(RequestEntidad $request){
        if (!Auth::user()->tieneFuncion($this->identificador_modulo, 'crear', $this->privilegio_superadministrador))
        ...
        $veterinaria->token = '';
        $veterinaria->entidad = 'si';
        $veterinaria->veterinaria = 'no';
        $veterinaria->save();
        ...
        $rol = Rol::where('entidades','si')->find($request->input('rol'));
        if(!$rol)
            return response(['error'=>['La información enviada es incorrecta']],422);
        ...
        if ($request->hasFile('logo')) {
            ...
            $nombre = str_replace('-','_',$nombre);
            $ruta = config('params.storage_img_entidades') . $veterinaria->id;
    ...
    public function editar($id)
    {
        ...
        $veterinaria = Veterinaria::where('entidad','si')->find($id);

        if(!$veterinaria || $veterinaria->estado == 'inactiva')
            return redirect('/');

        $roles = Rol::where('entidades','si')->pluck('nombre','id');
        return view('entidad.editar')
            ->with('identificador_modulo',$this->identificador_modulo)
            ->with('privilegio_superadministrador',$this->privilegio_superadministrador)
            ->with('roles',$roles)
    ...
    public function actualizar(RequestEntidad $request){
        if (!Auth::user()->tieneFuncion($this->identificador_modulo, 'editar', $this->privilegio_superadministrador) || !$request->has('veterinaria'))
            return response(['error'=>['La información enviada es incorrecta.']],422);

        $veterinaria  = Veterinaria::where('entidad','si')->find($request->input('veterinaria'));
        if (!$veterinaria || $veterinaria->estado == 'inactiva')
            return response(['error'=>['La información enviada es incorrecta.']],422);
