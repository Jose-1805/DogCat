...
function activar() {
    ...
    var url = $('#general_url').val()+'/veterinaria/activar';
    ...
    $.post(url,params)
        .done(function (data) {
            if(data.success){
                ...
                abrirAlerta('alertas-veterinarias','success',['La veterinaria ha sido activada con éxito'],null,'body');
...
function desactivar() {
    var params = {_token:$('#general_token').val(),veterinaria:veterinaria};
    var url = $('#general_url').val()+'/veterinaria/desactivar';
    ...
    $.post(url,params)
        .done(function (data) {
            if(data.success){
                ...
                veterinaria = null;
                cerrarBlockUiCargando();
                abrirAlerta('alertas-veterinarias','success',['La veterinaria ha sido desactivada con éxito'],null,'body');
                cargarTablaVeterinarias();
            }
        }).fail(function (jqXHR,state,error) {
            $('#modal-desactivar-veterinaria').modal('hide');
...
function cargarTablaVeterinarias() {
    ...
    tabla_veterinarias = $('#tabla-veterinarias').DataTable({
        ...
        processing: true,
        serverSide: true,
        ajax: $("#general_url").val()+"/veterinaria/lista",
        columns: cols,
