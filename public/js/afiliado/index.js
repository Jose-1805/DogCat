...
function activar() {
    ...
    var url = $('#general_url').val()+'/afiliado/activar';
    ...
    $.post(url,params)
        .done(function (data) {
            if(data.success){
                ...
                abrirAlerta('alertas-usuario','success',['EL afiliado ha sido activado con éxito'],null,'body');
                cargarTablaUsuarios();
            }
...
function desactivar() {
    var params = {_token:$('#general_token').val(),usuario:usuario};
    var url = $('#general_url').val()+'/afiliado/desactivar';
    ...
    $.post(url,params)
        .done(function (data) {
            if(data.success){
                ...
                usuario = null;
                cerrarBlockUiCargando();
                abrirAlerta('alertas-usuario','success',['El afiliado ha sido desactivado con éxito'],null,'body');
                cargarTablaUsuarios();
            }
        }).fail(function (jqXHR,state,error) {
        $('#modal-desactivar-usuario').modal('hide');
...
function cargarTablaUsuarios() {
    ...
    tabla_usuarios = $('#tabla-usuarios').DataTable({
        ...
        processing: true,
        serverSide: true,
        ajax: $("#general_url").val()+"/afiliado/lista",
        columns: cols,
